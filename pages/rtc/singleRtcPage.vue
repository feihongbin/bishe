<template>
	<view class="content" :style="{width: windowWidth + 'px',height: windowHeight + 'px'}">
		<!-- 观众加入房间后 暂无主播（提示） -->
		<view v-if="info.roleChoices == 2 && peeridInfo.length == 0" class="hint"
			:style="{top:(windowHeight/2-60) + 'px',width: windowWidth + 'px'}">
			<myLoading />
			<text class="text">暂无主播,请耐心等候主播加入</text>
		</view>
		<!-- 视频渲染(多人) -->
		<scroll-view scroll-y="true" class="CanvasView"
			style="{width: 1000px,height:800 px,paddingTop:  20px}">
			<!-- 本地视频渲染 -->
			<view :class="peeridInfoStyle" v-if="info.roleChoices != 2" style="position: relative;">
				<view style="flex: 1;">
					<AR-CanvasView ref="location" style="flex: 1;" />
				</view>
				<!-- 关闭视频后显示 -->
				<view class="location-none"
					:style="{left: video ? '-800px' : '0px',backgroundColor:  video ? '':'#0A1621',opacity: video ? '0':'1'}">
			...
			<!-- 远端用户视频渲染 -->
			<view v-for="peerid in peeridInfo" :key="peerid.uid" :class="peeridInfoStyle">
				<view style="flex: 1;">
		...
		<view class="options" :style="{width: windowWidth + 'px'}">
			...
			<view style="flex-direction: row;justify-content: space-between;margin-bottom: 20px;"
				...
				<!--直播模式 添加旁路推流  @click="liveTranscoFn"-->
				<view class="icon" @click="pullFn" v-if="info.scenario == 1">
					<image style="width: 70px;height: 70px;" src="../../static/save-fill.png" mode=""></image>
					<text class="icon_text">推流</text>
				</view>
				<!-- 前后摄像头转换 -->
			...
			<!-- 音频相关操作 -->
			<view :style="{flexDirection: 'row',justifyContent: info.roleChoices != 2 ? 'space-between':'center'}">
				<!-- 音频开关(非游客显示) -->
				<view class="icon" @click="audioFn" v-if="info.roleChoices != 2">
				...
				<!-- 扬声器开关(非游客显示) -->
				<view class="icon" @click="speakerphoneFn" v-if="info.roleChoices != 2">
		...
		<!-- 旁路推流相关 -->
		<view v-if="liveShow" class="live" :style="{width: windowWidth + 'px',height: windowHeight + 'px'}"
			@click="penetrateFn($event)">
			<view class="live-options" :style="{width: windowWidth + 'px'}">
				<text class="icon_text">注意：请确保当前使用的 appid 开通旁路推流服务，否则将推流失败</text>
				<input v-model="info.liveUrl" class="live-input" type="text" placeholder="请输入推流地址" />
				<view style="justify-content: space-around;flex-direction: row;margin-top: 60px;">
					<button style="width: 80px;" type="default" @click="liveShow = false">取消</button>
					<button style="width: 80px;" type="primary" @click="liveTranscoFn">确定</button>
				</view>
			</view>
		</view>
	</view>
		...
		methods: {
			...
			removePublishStreamUrl() {
				if (this.liveState) {
					...
					RTC.removePublishStreamUrl(this.info.liveUrl);
				} else {
					uni.showToast({
						title: "请先推流",
						icon: "none",
			...
			liveTranscoFn() {
				if (this.info.liveUrl) {
					this.liveShow = false;
					this.liveState = true;
					RTC.LiveTransco(this.info.liveUrl);
				} else {
					uni.showToast({
						title: "请输入推流地址",
						icon: "none",
			...
			async init() {
				...
				// 频道场景以及频道角色(默认通信场景)
				if (this.info.scenario == 1) {
					// 直播场景下设置频道角色
					const oRole = await RTC.setChannelProfile();
					console.log('直播场景下设置频道角色', oRole == 0 ? '成功' : '失败');
				}
				// 启用视频模块
