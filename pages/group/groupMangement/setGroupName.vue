	...
	<view class="nameContainer">
		<view class="chatTopBar">
			...
			<view class="chatFriendInfo">
				<text class="friendName">编辑群聊名称</text>
		...
		methods: {
			...
			getGroupInfo(groupId) {
				uni.request({
					...
					success: (data) => {
						...
						this.members = data.data.groupInfo.members
			...
			submit() {
				...
				if (this.name.length < 1) {
					...
					uni.request({
						url: this.$baseUrl + '/group/editName',
						method: 'post',
						data: {
					...
					that.members.forEach(item => {
						uni.getStorage({
							key:'accountId',
							success(res) {
								uni.request({
									url: that.$baseUrl + '/users/editGroupName',
									method: 'post',
									data: {
										account:item.member,
